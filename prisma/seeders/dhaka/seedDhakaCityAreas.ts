...
const AREAS: AreaSeed[] = [
  ...
  { zoneCode: 'ZONE-DNCC-UTTARA', code: 'AREA-DNCC-UTTARA-TONGI-BORDER', nameEn: 'Tongi Border', nameBn: 'টঙ্গি সীমানা' },
  { zoneCode: 'ZONE-DNCC-UTTARA', code: 'AREA-DNCC-UTTARA-UTTARA', nameEn: 'Uttara', nameBn: 'উত্তরা' },
  ...
  { zoneCode: 'ZONE-DNCC-BADDA', code: 'AREA-DNCC-BADDA-VATARA', nameEn: 'Vatara', nameBn: 'ভাটারা' },
  ...
  { zoneCode: 'ZONE-DSCC-KHILGAON', code: 'AREA-DSCC-KHILGAON-MUGDA', nameEn: 'Mugda', nameBn: 'মুগদা' },
...
export default async function seedDhakaCityAreas(
  ...
  corp: SeededCorp,
  zones: SeededZones,
  options?: { corp?: 'DNCC' | 'DSCC' },
) {
  const zoneIdByCode: Record<string, number> = { ...zones.dncc, ...zones.dscc };
  const corpFilter = options?.corp;
  const areas = corpFilter
    ? AREAS.filter((a) => a.zoneCode.startsWith(`ZONE-${corpFilter}-`))
    : AREAS;
