...
| **Accounting “integrate to Furtail ledger”** | No existing Furtail GL/journal API found. | Implement **posting pipeline only**: persist `PosJournalEntry` (or equivalent) with refType (ORDER/RETURN), refId, amounts (revenue, tax, discount, COGS, etc.), status (POSTED/FAILED), idempotency key. Admin/staff view for status + retry. **External sync to Furtail ledger = future adapter.** |
