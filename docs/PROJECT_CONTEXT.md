# Furtail (Furtail) – Project Context

## Overview
Furtail is a national animal welfare & pet ecosystem platform. It connects pet parents, clinics, pet shops, delivery hubs, staff, and admins.
...
- Frontend:
  ...
  - **furtail-landing** — Next.js marketing site (apex domain)
  - **vaccination_2026** — Next.js campaign booking site (subdomain)
  - **furtail_app** — Flutter mobile app (Riverpod state management)
...
|------------|------|------------|-----------------|
| `backend-api` | Central API | 3000 | `api.furtail.world` |
| `furtail-landing` | Marketing / SEO | 3101 | `furtail.world` |
| `vaccination_2026` | Campaign booking | 3110 | `vaccination.furtail.world` |
| `bpa_web` | Admin & branch panels | 3100–3107 | `admin.`, `shop.`, `clinic.`, etc. |
| `furtail_app` | Mobile client | — | App stores (HTTPS → API) |
...
- **furtail-landing:** **3101** (conflicts locally with bpa_web shop — see port map)
...
`furtail-landing` and `bpa_web` shop both use port **3101**. On one machine, run only one at a time, or use a temporary port override for shop (documented in PORT_AND_DOMAIN_MAP). Production uses separate containers/upstreams per host.

## API
- Base URL: http://localhost:3000/api/v1
- Production: https://api.furtail.world/api/v1
...
- **Multi-domain production:** [./architecture/furtail-vaccination-domain-strategy.md](./architecture/furtail-vaccination-domain-strategy.md), [./nginx-production-deployment.md](./nginx-production-deployment.md).
