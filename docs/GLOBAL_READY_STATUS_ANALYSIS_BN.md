...
| **Country রিজলভ** | ✅ করা | `lib/countryContext.ts` – subdomain (bd., in., us., ae.) → localStorage `furtail_country_code` → ডিফল্ট BD। `getCountryCode()`, `setCountryCode()`, `getApiHeaders()`। |
...
### ৩.৩ Flutter অ্যাপ (furtail_app)

- ডকুমেন্ট অনুযায়ী: প্রথম লঞ্চে country picker, `furtail_country_code` পারসিস্ট, সব রিকোয়েস্টে `X-Country-Code`; policy অনুযায়ী Donation/Ads এন্ট্রি হাইড।
...
5. **furtail_app:** ডক ও চেকলিস্ট অনুযায়ী country picker, X-Country-Code ও policy-based UI ভেরিফাই করা।
